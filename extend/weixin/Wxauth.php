		...
		if(!$code && isset($_SESSION['open_id']) && isset($_SESSION['user_token']) && $token_time>time()-3600){
			...
			if ($code) {
				...
				if (!$json) {
					...
					return array('status'=>0,'msg'=>'获取用户授权失败，请重新确认.');
				}
				...
				if ($userinfo && !empty($userinfo['nickname'])) {
					...
					if ($userinfo && !empty($userinfo['nickname'])) {
						...
						$this->wxuser = array(
							'subscribe'=>(int)$userinfo['subscribe'],
							'open_id'=>$this->open_id,
							'nickname'=>$userinfo['nickname'],
							'sex'=>intval($userinfo['sex']),
							'location'=>$userinfo['province'].'-'.$userinfo['city'],
				...
				if ($this->wxuser) {
					...
					unset($_SESSION['wx_redirect']);
					// 直接跳转到没有 code 参数的url 这样页面刷新也不会有问题了。
					$iuri = 0;
					foreach($_GET as $key=>$item){
						if($iuri >1 && $key!='code' && $key!='state'){
							$myurl[] = $key.'='.$item;
						}
						$iuri++;
					}
					$options = '';
					if(count($myurl) > 0)$options = '?'.join('&', $myurl);
					$wx_redirect = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REDIRECT_URL'].$options;
					redirect ( $wx_redirect );
				}
			...
			if (!$url) {
				unset($_SESSION['wx_redirect']);
				return array('status'=>0,'msg'=>'获取用户授权失败.');
			}
