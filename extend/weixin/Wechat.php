...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if ($result)
		{
			...
			$expire = $json['expires_in'] ? intval($json['expires_in'])-500 : 3200;
	 ...
	 * struct  "gps":{
			...
			"time": NumberInt(1466352000)
	   },
	 *dateTime 2016-11-18 
     */
     public function getgpsinfo($latitude,$longitude){
     	$geoconvurl='http://api.map.baidu.com/geoconv/v1/?coords='.$longitude.','.$latitude.'&from=1&to=5&ak=G2Z9ANn87HO8MG26e9ySZvwA&callback=?';
		$geodata=json_decode($this->http_get($geoconvurl),true);
		$url= 'http://api.map.baidu.com/geocoder/v2/?output=json&pois=1&location='.$geodata['result'][0]['y'].','.$geodata['result'][0]['x'].'&ak=G2Z9ANn87HO8MG26e9ySZvwA&callback=?';
        $data=json_decode($this->http_get($url),true);
        if($data['status']==0)
        {
        	 $gps=array('location'=>array('lng'=>$data['result']['location']['lng'],'lat'=>$data['result']['location']['lat']),
